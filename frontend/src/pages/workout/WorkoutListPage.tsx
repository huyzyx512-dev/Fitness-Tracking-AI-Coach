...
export default function WorkoutListPage() {
  ...
  const columns: Column<Workout>[] = [
    ...
    },
    {
      ...
      header: "Bài tập",
      render: (w) => (
        <span className="text-muted">{w.exercises?.length ?? 0} bài</span>
      ),
    },
    {
      key: "scheduled_at",
      ...
      sortable: true,
      render: (w) => (
        <span className="text-muted text-sm">{formatDate(w.scheduled_at)}</span>
      ),
    },
    {
      key: "log",
        ...
        return (
          <span className="text-muted text-sm">
            <span className="tabular-nums">
              {formatDuration(energy.totalDurationMinutes)}
            </span>
            {!energy.isActualTotals && (
              <span className="block text-[10px] text-subtle leading-tight mt-0.5">
                Ước tính
              </span>
            )}
          </span>
          ...
          items={[
            {
              label: "Xem chi tiết",
              icon: <Eye size={13} />,
              onClick: () => navigate(ROUTES.WORKOUT_DETAIL(w.id)),
            },
            {
              label: "Chỉnh sửa",
              icon: <Pencil size={13} />,
              onClick: () => navigate(ROUTES.WORKOUT_EDIT(w.id)),
